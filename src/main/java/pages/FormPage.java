...
public class FormPage extends AbsBasePage {
    ...
    public void validateOutputData(String username,
                                   String email,
                                   String date,
    ...
    private void verifyOutputField(String fullText, String fieldLabel, String expectedValue) {
        Pattern pattern = Pattern.compile(fieldLabel + "\\s*(.*?)(?:\\s*<br>|$)");
        Matcher matcher = pattern.matcher(fullText);

        if (!matcher.find() || !matcher.group(1).trim().equals(expectedValue)) {
            throw new AssertionError(fieldLabel + " validation failed. Expected: " +
                    expectedValue + ", Actual: " +
                    (matcher.find() ? matcher.group(1).trim() : "not found"));
        }
    }
